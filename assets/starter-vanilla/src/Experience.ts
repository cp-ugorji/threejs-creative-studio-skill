...
export class Experience {
  ...
  constructor(private readonly canvas: HTMLCanvasElement) {
    ...
    this.scene.fog = new THREE.FogExp2('#090b12', 0.055)
  ...
  setView(name: string) {
    this.targetCamera.copy(
      new THREE.Vector3(...(VIEW_POSITIONS[name] ?? VIEW_POSITIONS.front)),
    )
  }
  ...
  private buildScene() {
    const platform = new THREE.Mesh(
      new THREE.CylinderGeometry(2.1, 2.35, 0.25, 64),
      new THREE.MeshStandardMaterial({ color: '#171c2a', roughness: 0.52, metalness: 0.35 }),
    )
    ...
    this.scene.add(new THREE.HemisphereLight('#8eb7ff', '#1c1027', 1.3))

    const key = new THREE.DirectionalLight('#dce9ff', 4.5)
    ...
    const rim = new THREE.PointLight('#ff4fd8', 28, 8, 2)
